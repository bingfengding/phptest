...
<table>
    ...
    while ($row = mysqli_fetch_array($result,MYSQLI_ASSOC)){
        ...
        echo $row["proimg"]=="图片暂缺"? "<td>图片暂缺</td>":"<td><img src='".$row["proimg"]."'></td>";
        echo "<td>",$row["prourl"],"</td>";
        echo "</tr>";
    }
    ?>
</table>
<a href="showshop.php">首页</a>
<a href=<?php
if(!isset($_GET['pagenow'])||$pagenow==1){
    echo "showshop.php?pagenow=1";
}else{
    echo "showshop.php?pagenow=".($pagenow-1);
}
?>>上一页</a>
<a href=<?php
if(!isset($_GET['pagenow']) && $pagecount==1){
    ...
    echo "showshop.php?pagenow=2";
}elseif(isset($_GET['pagenow'])&&$pagenow==$pagecount){
    echo "showshop.php?pagenow=".$pagecount;
}else{
    echo "showshop.php?pagenow=".($pagenow+1);
}
...
<ul>
    ...
    for ($i=1;$i<=$pagecount;$i++){
        echo "<li onclick='location.href=\"showshop.php?pagenow=$i\"'>".$i."</li>";
    }
    ?>
</ul>
<?php
